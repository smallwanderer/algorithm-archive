...
def bfs():
  ...
  while True:
    ...
    #   if 0 <= ny < height and 0 <= nx < width:
    #     # if different swan_type return day
    #     if dp[current[0]][current[1]][1] != dp[ny][nx][1] and dp[ny][nx][0] != -1:
    #       return max(dp[current[0]][current[1]][0], dp[ny][nx][0])
    #     if grid[ny][nx] == ".": # if water, no day count and copy swan_type
    #       if dp[ny][nx][0] == -1:
    #         queue.appendleft((ny, nx))
    #         dp[ny][nx] = dp[current[0]][current[1]]
    #     elif grid[ny][nx] == "X": # if ice, day count and copy swan_type
    #       if dp[ny][nx][0] == -1:
    #         queue.append((ny, nx))
    #         dp[ny][nx] = (dp[current[0]][current[1]][0] + 1, dp[current[0]][current[1]][1])
